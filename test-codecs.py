...
for video_codec in video_codecs:
    ...
    if not len(contents) == 0:
        run_number = int(sorted(contents, reverse=True)[0]) + 1
    ...
    for i in range(num_iterations):
        ...
        command = "ffmpeg -hide_banner -loglevel error -i {} -c:v {} -c:a {} {}".format(input_file, video_codec, audio_codec, out_dir + output_file)

        print("{}: {}".format(video_codec, i))
        ...
        times.append((i, end_time - start_time))
